...
@ 아래 트러블슈팅은 cloud-init의 기능과 충돌이 일어났을때와 증상이 같으므로 만약 다음 모든증상들과 동일하다면,
충돌이 일어난것이므로 다음 해결방법 말고 virt-customize 스크립트 설정을 점검해야함
...
**문제**: SSH 접속 시 호스트 키 관련 오류가 발생하거나 SSH 서비스가 시작되지 않는 경우

**원인**: `virt-customize`로 클라우드 이미지를 수정하면서 SSH 호스트 키가 생성되지 않았을 수 있음
